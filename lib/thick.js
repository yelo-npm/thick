...
var processOptions = function(options){
	...
	var flattenedLocals = flattenObject(options.locals);
	_.merge(opts.locals,options.locals,flattenedLocals);
	_.merge(opts.options,options.options);
}

var flattenObject = function(ob,join){
	if(!join){join = '_';}
	var toReturn = {};
	for (var i in ob) {
		...
		if ((typeof ob[i]) == 'object') {
			...
			for (var x in flatObject) {
				...
				toReturn[i + join + x] = flatObject[x];
			}
		} else {
			toReturn[i] = ob[i];
		}
